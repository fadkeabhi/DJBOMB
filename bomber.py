...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("DJBomb was updated to the latest version")
        mesgdcrt.GeneralMessage("Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update DJBomb.")
        mesgdcrt.WarningMessage("Grab The Latest one From https://github.com/fadkeabhi/DJBOMB")
...
def do_git_update():
    ...
    if success:
        ...
        print("git checkout . && git pull https://github.com/fadkeabhi/DJBomb.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get("https://raw.githubusercontent.com/fadkeabhi/DJBomb/master/.version").text.strip()
    if fver != __VERSION__:
        mesgdcrt.WarningMessage("An update is available")
        mesgdcrt.GeneralMessage("Starting update...")
        update()
    else:
        mesgdcrt.SuccessMessage("DJBomb is up-to-date")
        mesgdcrt.GeneralMessage("Starting DJBomb")

def notifyen():
    try:
        noti = requests.get("https://raw.githubusercontent.com/fadkeabhi/DJBomb/master/.notify").text.upper().strip()
        if len(noti) > 10:
...
def pretty_print(cc,target,success,failed):
    ...
    mesgdcrt.SuccessMessage("DJBomb was Modified by DJABRJ")
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit={"sms":5000000000,"call":150000000,"mail":20000000}
        cc,target="",""
        if mode in ["sms","call"]:
...
if sys.version_info[0]!=3:
    mesgdcrt.FailureMessage("DJBomb will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['DJABRJ']

ALL_COLORS = [Fore.GREEN, Fore.RED, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN, Fore.WHITE]
RESET_ALL = Style.RESET_ALL

description="""DJBomb - Your Friendly Spammer Application

DJBomb can be used for many purposes which incudes - 
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

DJBomb is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,epilog='Coded by SpeedX !!!')
parser.add_argument("-sms","--sms", action="store_true",help="start DJBomb with SMS Bomb mode")
parser.add_argument("-call","--call", action="store_true",help="start DJBomb with CALL Bomb mode")
parser.add_argument("-mail","--mail", action="store_true",help="start DJBomb with MAIL Bomb mode")
parser.add_argument("-u","--update", action="store_true",help="update DJBomb")
parser.add_argument("-c","--contributors", action="store_true",help="show current DJBomb contributors")
parser.add_argument("-v","--version", action="store_true",help="show current DJBomb version")
